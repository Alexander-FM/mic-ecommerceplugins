...
public class EmployeeServiceImpl implements EmployeeService {
  ...
  public GenericResponse<List<EmployeeResponseDto>> getAllEmployees() {
    List<Employee> employees = this.repository
        .findAll()
        .stream()
        .toList();
    final List<Integer> userIds = employees
        .stream()
    ...
    return GenericUtils.buildGenericResponseSuccess(EmployeeConstants.FIND_MESSAGE, employees
        .stream()
        .map(employee -> {
  ...
  public GenericResponse<EmployeeResponseDto> findById(final Integer id) {
    ...
    final Optional<User> user = this.userRepository.findById(employee
        .get()
        .getUserId());
    if (user.isEmpty()) {
  ...
  @Override
  public GenericResponse<EmployeeResponseDto> findByUsername(String username) {
    Optional<User> user = this.userRepository.findByUsername(username);
    if (user.isEmpty()) {
      return GenericUtils.buildGenericResponseError(EmployeeConstants.NOT_EXIST_USER_FOR_EMPLOYEE, null);
    }
    Optional<Employee> employee = this.repository.findByUserId(user
        .get()
        .getId());
    if (employee.isEmpty()) {
      return GenericUtils.buildGenericResponseError(EmployeeConstants.NO_EXIST, null);
